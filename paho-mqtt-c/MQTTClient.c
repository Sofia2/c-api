 ...
 * 	  Carlo Gavazzi Automation S.p.A. - fix for potential bug - gettimeofday() calls
 	  can lead to locks if a process changes the current time while the MQTT client 
	  is waiting for incoming data.
*******************************************************************************/
...
int MQTTClient_connectURI(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI)
{
	...
	if (m->c->sslopts)
	{
		if (m->c->sslopts->cafile)
			free((void*)m->c->sslopts->cafile);
	...
	if (options->struct_version != 0 && options->ssl)
	{
		...
		if (options->ssl->cafile)
			m->c->sslopts->cafile = MQTTStrdup(options->ssl->cafile);
...
void MQTTClient_freeOpenSSLTables(){
#if defined(OPENSSL)
    ERR_clear_error();
    ERR_remove_state(0);
    ENGINE_cleanup();
